# admin_app.py - Complete Admin Dashboard
...
# ==================== MANUAL COSTS ====================
def init_manual_costs_table():
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute("""CREATE TABLE IF NOT EXISTS manual_costs (id SERIAL PRIMARY KEY, cost_type VARCHAR(50) NOT NULL, amount DECIMAL(10,2) NOT NULL, month DATE NOT NULL, notes TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""")
        conn.commit()
        conn.close()
    except Exception as e:
        print(f"Manual costs table error: {e}")
