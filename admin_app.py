# admin_app.py - Complete Admin Dashboard with Error Tracking
...
def init_tables():
    try:
        ...
        # Health check logs
        cur.execute("""CREATE TABLE IF NOT EXISTS health_checks (
            id SERIAL PRIMARY KEY,
            service VARCHAR(50),
            status VARCHAR(20),
...
def fetch_error_metrics():
    try:
        ...
        cur = conn.cursor()
        # Error counts
        cur.execute("SELECT COUNT(*) as count FROM error_logs WHERE created_at >= CURRENT_DATE - INTERVAL '24 hours'")
        errors_24h = cur.fetchone()['count']
        cur.execute("SELECT COUNT(*) as count FROM error_logs WHERE created_at >= CURRENT_DATE - INTERVAL '7 days'")
        errors_7d = cur.fetchone()['count']
        cur.execute("SELECT COUNT(*) as count FROM error_logs WHERE resolved = FALSE")
        unresolved = cur.fetchone()['count']
        # Recent errors
        cur.execute("""SELECT id, error_type, message, endpoint, user_code, severity, resolved, created_at 
                       FROM error_logs ORDER BY created_at DESC LIMIT 50""")
        recent_errors = cur.fetchall()
        # Errors by type
        cur.execute("""SELECT error_type, COUNT(*) as count FROM error_logs 
                       WHERE created_at >= CURRENT_DATE - INTERVAL '7 days' 
                       GROUP BY error_type ORDER BY count DESC LIMIT 10""")
        by_type = cur.fetchall()
        # Errors by endpoint
        cur.execute("""SELECT endpoint, COUNT(*) as count FROM error_logs 
                       WHERE created_at >= CURRENT_DATE - INTERVAL '7 days' 
        ...
        return {
            ...
            'recent': [dict(e) for e in recent_errors],
            'by_type': [dict(t) for t in by_type],
            'by_endpoint': [dict(ep) for ep in by_endpoint]
        }
    except Exception as e:
        return {'errors_24h': 0, 'errors_7d': 0, 'unresolved': 0, 'recent': [], 'error': str(e)}
...
def get_all_metrics():
    ...
    try:
        ...
        error_metrics = fetch_error_metrics()
...
@app.route('/api/errors/resolve/<int:error_id>', methods=['POST'])
def resolve_error(error_id):
    ...
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute("UPDATE error_logs SET resolved = TRUE WHERE id = %s", (error_id,))
        conn.commit()
        conn.close()
        return jsonify({'success': True})
    except Exception as e:
...
@app.route('/api/errors/resolve-all', methods=['POST'])
def resolve_all_errors():
    ...
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute("UPDATE error_logs SET resolved = TRUE WHERE resolved = FALSE")
        conn.commit()
        conn.close()
        return jsonify({'success': True})
    except Exception as e:
...
<div id="customers-tab" class="tab-content"><div class="grid"><div class="card"><h3>Total Users</h3><div class="value"><span id="cust-total">-</span></div></div><div class="card"><h3>Active (1+ login, 7d)</h3><div class="value"><span id="cust-active-once">-</span></div><div class="subvalue"><span id="cust-active-once-pct">-</span>%</div></div><div class="card"><h3>Active (3+ logins, 7d)</h3><div class="value"><span id="cust-active-thrice">-</span></div><div class="subvalue"><span id="cust-active-thrice-pct">-</span>%</div></div><div class="card"><h3>Growth Rate (7d)</h3><div class="value" id="cust-growth">-</div><div class="subvalue"><span id="cust-signups-7d">-</span> new</div></div><div class="card"><h3>Retention (7d)</h3><div class="value" id="cust-retention">-</div></div></div><div class="section"><h2>📊 Database</h2><table><tr><td>Size</td><td><strong><span id="db-size">-</span> GB</strong></td></tr><tr><td>Active Connections</td><td><strong><span id="db-connections">-</span></strong></td></tr></table></div><div class="section"><h2>📞 Twilio</h2><table><tr><td>Total Calls (30d)</td><td><strong><span id="twilio-calls">-</span></strong></td></tr><tr><td>Total Duration</td><td><strong><span id="twilio-duration">-</span> min</strong></td></tr><tr><td>Avg Duration</td><td><strong><span id="twilio-avg">-</span> min</strong></td></tr><tr><td>Balance</td><td><strong>$<span id="twilio-balance">-</span></strong></td></tr></table></div><div class="section"><h2>🤖 AI Usage</h2><table><tr><td>Queries</td><td><strong><span id="gemini-queries">-</span></strong></td></tr><tr><td>Scans</td><td><strong><span id="gemini-scans">-</span></strong></td></tr><tr><td>Total Tokens</td><td><strong><span id="gemini-tokens">-</span></strong></td></tr></table></div><div class="section"><h2>📈 DAU (30d)</h2><div class="chart-container"><canvas id="dau-chart"></canvas></div></div></div>
<div id="errors-tab" class="tab-content"><div class="grid"><div class="card"><h3>Errors (24h)</h3><div class="value negative" id="err-24h">-</div></div><div class="card"><h3>Errors (7d)</h3><div class="value negative" id="err-7d">-</div></div><div class="card"><h3>Unresolved</h3><div class="value neutral" id="err-unresolved">-</div></div></div><div class="section"><h2>📊 Errors by Type</h2><table><thead><tr><th>Error Type</th><th>Count</th></tr></thead><tbody id="err-by-type"><tr><td colspan="2" style="text-align:center;color:#888">Loading...</td></tr></tbody></table></div><div class="section"><h2>📍 Errors by Endpoint</h2><table><thead><tr><th>Endpoint</th><th>Count</th></tr></thead><tbody id="err-by-endpoint"><tr><td colspan="2" style="text-align:center;color:#888">Loading...</td></tr></tbody></table></div><div class="section"><h2>🚨 Recent Errors <button class="btn btn-sm btn-danger" onclick="resolveAll()">Resolve All</button></h2><table><thead><tr><th>Time</th><th>Type</th><th>Message</th><th>Endpoint</th><th>Status</th><th>Action</th></tr></thead><tbody id="err-recent"><tr><td colspan="6" style="text-align:center;color:#888">Loading...</td></tr></tbody></table></div></div>
